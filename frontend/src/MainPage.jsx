...
function MainPage() {
  ...
  const uploadImage = async (event) => {
    ...
    try {
      ...
      // 1. 헤더에서 정보 추출
      const extraInfoRaw = response.headers['x-extra-info'];
      const extraInfo = extraInfoRaw ? JSON.parse(extraInfoRaw) : {};

      // 2. 상태에 저장 (즉시 다운로드하지 않음)
      setSummaryData(response.data);

      console.log(response.data,'데이터 수신 완료. 이제 드롭다운에서 다운로드 가능합니다.');
      ...
      {summaryData && (
        <div style={{ position: 'relative', width: '80%', margin: '0 auto', textAlign: 'left', background: '#fff', padding: '20px', borderRadius: '8px', border: '1px solid #ddd' }}>
          ...
          <div style={{ marginTop: '10px', whiteSpace: 'pre-wrap', backgroundColor: '#f9f9f9', padding: '15px' }}>
             ...
             {summaryData.summary || "내용을 서버에서 가져왔습니다."}
          </div>
        </div>
      )}
